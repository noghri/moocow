...
sqlite3 $FILENAME <<EOF
	...
	CREATE TABLE tbscores (nick unique, wins, cheats, losses, timeouts, duds, doubles, backfires, id integer primary key autoincrement);
	...
	CREATE TABLE users (username unique, access, wzdefault, trdefault , userid integer primary key autoincrement);
	CREATE TABLE usermask (hostmask, userid integer not null, maskid integer primary key autoincrement, foreign key(userid) REFERENCES users(userid) ON DELETE CASCADE);
